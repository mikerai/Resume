...
const { user, profile, updateProfileLocally: getProfile } = useAuth();
...
const onAvatarSelect = async (event) => {
    ...
    try {
        ...
        // Update profile in DB (supplier_profiles)
...
const updatePassword = async () => {
    ...
    try {
        ...
        toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo actualizar la contraseña', life: 3000 });
    } finally {
        loadingProfile.value = false;
    }
};
...
// Helper functions from AppConfigurator
function getPresetExt() {
    ...
    if (color.name === 'noir') {
        return {
            semantic: {
                primary: {
                    50: '{surface.50}', 100: '{surface.100}', 200: '{surface.200}', 300: '{surface.300}', 400: '{surface.400}', 500: '{surface.500}', 600: '{surface.600}', 700: '{surface.700}', 800: '{surface.800}', 900: '{surface.900}', 950: '{surface.950}'
                },
                colorScheme: {
                    light: {
                    ...
                    dark: {
                        ...
                        highlight: { background: '{primary.50}', focusBackground: '{primary.300}', color: 'rgba(255,255,255,.87)', focusColor: 'rgba(255,255,255,.87)' }
                    }
                }
            }
        };
...
// Load profile info (names) then avatar
onMounted(() => {
    if (user.value?.id) {
        getProfile(user.value.id).then(() => {
            loadAvatar();
        }).catch(e => {
            console.error('Error loading profile on Settings mount:', e);
            loadAvatar();
        });
    } else {
        loadAvatar();
                ...
                <template #content>
                    <div class="flex flex-col items-center mb-6">
                        <div class="relative w-32 h-32 mb-4">
                            <img :src="avatarUrl || 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y'" alt="Avatar" class="w-full h-full rounded-full object-cover border-2 border-primary" />
                    ...
                    <div class="field mb-4">
                        ...
                        <Password v-model="newPassword" toggleMask class="w-full" inputClass="w-full" :feedback="true" />
                    </div>
                    <div class="field mb-4">
                        <label class="font-medium block mb-2">Confirmar Contraseña</label>
                        <Password v-model="confirmPassword" toggleMask class="w-full" inputClass="w-full" :feedback="false" />
                    </div>
                    <Button label="Actualizar Contraseña" icon="pi pi-lock" @click="updatePassword" :loading="loadingProfile" class="w-full" />
                </template>
            </Card>
        </div>
        ...
        <div class="col-span-12 lg:col-span-6">
            <Card>
                ...
                <template #content>
                    <!-- Mode -->
                    <div class="mb-6">
                        <span class="block font-medium mb-3">Modo</span>
                        <div class="flex gap-4">
                            <Button icon="pi pi-sun" label="Claro" :outlined="isDarkTheme" @click="layoutConfig.darkTheme = false; toggleDarkMode()" class="flex-1" />
                            <Button icon="pi pi-moon" label="Oscuro" :outlined="!isDarkTheme" @click="layoutConfig.darkTheme = true; toggleDarkMode()" class="flex-1" />
                        </div>
                    </div>

                    <!-- Primary Color -->
                    <div class="mb-6">
                        <span class="block font-medium mb-3">Color Principal</span>
                        <div class="flex flex-wrap gap-2">
                            <button v-for="primaryColor of primaryColors" :key="primaryColor.name" type="button" :title="primaryColor.name" @click="updateColors('primary', primaryColor)" :class="['border-none w-6 h-6 rounded-full p-0 cursor-pointer outline-none outline-offset-1', { 'outline-primary': layoutConfig.primary === primaryColor.name }]" :style="{ backgroundColor: `${primaryColor.name === 'noir' ? 'var(--text-color)' : primaryColor.palette['500']}` }"></button>
                        </div>
                    </div>

                    <!-- Surface Color -->
                    <div class="mb-6">
                        <span class="block font-medium mb-3">Superficie</span>
                        <div class="flex flex-wrap gap-2">
                            <button v-for="surface of surfaces" :key="surface.name" type="button" :title="surface.name" @click="updateColors('surface', surface)" :class="['border-none w-6 h-6 rounded-full p-0 cursor-pointer outline-none outline-offset-1', { 'outline-primary': layoutConfig.surface ? layoutConfig.surface === surface.name : isDarkTheme ? surface.name === 'zinc' : surface.name === 'slate' }]" :style="{ backgroundColor: `${surface.palette['500']}` }"></button>
                        </div>
                    </div>

                    <!-- Presets -->
                    <div class="mb-6">
                        <span class="block font-medium mb-3">Tema Base</span>
