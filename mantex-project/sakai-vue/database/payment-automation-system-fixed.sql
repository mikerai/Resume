-- ==============================================
-- MANTEX - SISTEMA DE AUTOMATIZACIÓN DE PAGOS 48H (CORREGIDO)
-- ==============================================
...
-- ==============================================
-- FUNCIONES DE AUTOMATIZACIÓN (CORREGIDAS)
-- ==============================================
...
DECLARE
    ...
    work_date TIMESTAMPTZ; -- Cambié de current_date a work_date
    days_to_add INTEGER;
    hours_remaining INTEGER;
BEGIN
    work_date := approved_at;
    ...
    WHILE hours_remaining > 0 LOOP
        ...
        IF exclude_weekends AND EXTRACT(DOW FROM work_date) IN (0, 6) THEN
            work_date := work_date + ((8 - EXTRACT(DOW FROM work_date))::INTEGER * INTERVAL '1 day');
            work_date := date_trunc('day', work_date) + INTERVAL '9 hours'; -- 9 AM
        END IF;

        -- Si es horario laboral (9 AM - 6 PM), procesar horas
        IF business_hours_only THEN
            DECLARE
                current_hour INTEGER := EXTRACT(HOUR FROM work_date);
            ...
            BEGIN
                ...
                IF current_hour < 9 OR current_hour >= 18 THEN
                    work_date := date_trunc('day', work_date) + INTERVAL '1 day' + INTERVAL '9 hours';
                    CONTINUE;
                END IF;

                -- Calcular horas disponibles hasta las 6 PM
                available_hours := 18 - current_hour;
                hours_to_use := LEAST(hours_remaining, available_hours);

                work_date := work_date + (hours_to_use * INTERVAL '1 hour');
        ...
        ELSE
            ...
            work_date := work_date + (hours_remaining * INTERVAL '1 hour');
            hours_remaining := 0;
        END IF;
    END LOOP;
...
-- Función simplificada para procesar jobs de pago pendientes
...
BEGIN
    ...
    FOR job_record IN
        ...
        LIMIT 50 -- Limitar a 50 jobs por ejecución
    LOOP
        ...
        BEGIN
            ...
            CASE job_record.job_type
                ...
                WHEN 'notify' THEN
                    ...
                    NULL; -- Las notificaciones se manejarán en el frontend

                ELSE
                    -- Tipo de job no reconocido
                    RAISE NOTICE 'Tipo de job no reconocido: %', job_record.job_type;
            END CASE;
